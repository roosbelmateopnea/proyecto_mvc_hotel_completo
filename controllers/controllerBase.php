...
class ControllerBase {
    ...
    public function validateData($datos){
        ...
        if(empty(trim($datos['document_type_id'] ?? ''))){
            $errores['document_type_id'] = 'Tipo de documento requerido';
        }
        if(!preg_match('/^[0-9]+$/', $datos['document_number'])){
        ...
        if(empty(trim($datos['phone'] ?? ''))){
            $errores['phone'] = 'Teléfono requerido';
        }
